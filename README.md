...
```bash
conda create -n adb_dbx_demo python=3.9
conda activate adb_dbx_demo
```
...
```
This command will create your personal repository under `/Repos/<username>/adb_dbx_demo`.
...
- To trigger the release pipeline, get the current version from the `adb_dbx_demo/__init__.py` file and tag the current code version:
```
